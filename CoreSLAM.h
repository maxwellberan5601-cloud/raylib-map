...
#define TS_SCAN_SIZE 8192 //max number of reading per scan
#define TS_MAP_SIZE 2048 //maps is 2048x2048
#define TS_MAP_SCALE 0.1 //cell conversion scale from mm t cells TS_MAP_SCALE 1/10mm or 1/10 therefore 500mm = 50 grid cells
#define TS_NO_OBSTACLE 65500 //map scoring
#define TS_OBSTACLE 0

typedef unsigned short ts_map_pixel_t; //different way of writing uint16_t = unsigned short, uint16_t = ts_map_pixel_t
typedef struct {
    ts_map_pixel_t map[TS_MAP_SIZE * TS_MAP_SIZE]; //the map is an array of uint16_t   which just decided TS_MAP_SIZE is the end of a row 
} ts_map_t;

typedef struct {
    double x[TS_SCAN_SIZE], y[TS_SCAN_SIZE]; //local lidar x y
    int value[TS_SCAN_SIZE]; //wether the point is occpied or free space 0 - 65500
    int nb_points; //number of vaild entries
} ts_scan_t;
