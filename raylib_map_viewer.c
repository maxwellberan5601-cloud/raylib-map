...
// static char g_openedSerialPath[64] = "";

// static int ConfigureSerialPort(int fd, const char *device)
// {
//     struct termios tty;
...
// static int TryOpenSerialPort(const char *device, bool printError)
// {
...
//     if (ConfigureSerialPort(fd, device) < 0)
//     {
//         return -1;
//     }

//     snprintf(g_openedSerialPath, sizeof(g_openedSerialPath), "%s", device);
//     return fd;
// }

// static int OpenSerialPortAuto(const char *requestedDevice)
// {
//     if (requestedDevice != NULL)
//     {
//         int fd = TryOpenSerialPort(requestedDevice, true);
//         if (fd < 0)
//         {
//             fprintf(stderr, "In WSL, Windows COM9 is sometimes /dev/ttyS9, but WSL2 often needs USB passthrough first.\n");
//             fprintf(stderr, "After USB passthrough, Arduino-style boards usually appear as /dev/ttyACM0 or /dev/ttyUSB0.\n");
//             fprintf(stderr, "Check devices with: ls /dev/ttyS* /dev/ttyUSB* /dev/ttyACM*\n");
//             fprintf(stderr, "If this is a permission problem, try `sudo usermod -aG dialout $USER` then restart WSL.\n");
//         }
//         return fd;
//     }
...
//         "/dev/ttyS9", "/dev/ttyS4", "/dev/ttyS3", "/dev/ttyS0"
//     };

//     for (int i = 0; i < (int)(sizeof(commonPorts) / sizeof(commonPorts[0])); i++)
//     {
//         int fd = TryOpenSerialPort(commonPorts[i], false);
//         if (fd >= 0)
//         {
//             return fd;
//         }
...
//     fprintf(stderr, "Could not find an open serial port.\n");
//     fprintf(stderr, "Check devices with: ls /dev/ttyS* /dev/ttyUSB* /dev/ttyACM*\n");
//     fprintf(stderr, "For WSL2, attach the USB serial device with usbipd from Windows first.\n");
//     fprintf(stderr, "Then run `./map /dev/ttyACM0` or `./map /dev/ttyUSB0`, depending on what appears.\n");
//     return -1;
// }

// /* Store one freshly-received integer into its slot in the cycling grid */
// static void StoreValue(int value)
// {
...
// /* Parse a line; only accept it if the whole (trimmed) line is a valid integer.
//  * Anything else (blank lines, "END", debug text, etc.) is ignored. */
...
//     if (endptr == p) return; /* no digits parsed at all */

//     while (*endptr == ' ' || *endptr == '\t' || *endptr == '\r') endptr++;
//     if (*endptr != '\0') return; /* trailing junk -> not a clean integer line */
...
// {
//     const char *requestedPortPath = (argc > 1) ? argv[1] : NULL;
//     g_serialFd = OpenSerialPortAuto(requestedPortPath);
//     if (g_serialFd < 0)
//     {
...
//         printf("Expecting one clean integer per line at 115200 baud.\n");
//     }
...
//         DrawText(TextFormat("[%s] Each cell = %d mm x %d mm. Values received: %lu | shown range: %d (black) - %d (white)",
//                              status, CELL_SIZE_MM, CELL_SIZE_MM, g_receivedCount, g_minValue, g_maxValue),
...
// }
//complie code
//gcc -Wall -Wno-unused-function -o map raylib_map_viewer.c -lraylib -lGL -lm -lpthread -ldl -lrt -lX11
//run: ./map                 (uses default port /dev/ttyACM0)
//run: ./map /dev/ttyUSB0    (override serial port)
//
// NOTE: reading a serial port needs permission on Linux. If open() fails
// with "Permission denied", either run with sudo or add your user to the
// dialout group: `sudo usermod -aG dialout $USER` then log out/in.

/*
 * raylib_map_viewer.c
 * ---------------------------------------------------------
 * Reads a live stream from a serial port and displays it in
 * real time as a colored grid.
 *
 * Expected line format (one full grid per line):
 *   MAP:(0,0),(1,489),(2,978),...,(255,124695)
 * Each "(i,val)" pair means: put val in grid slot i, where
 *   i = 0                      -> BOTTOM-LEFT cell
 *   i increases left-to-right across a row, then row-by-row up
 *   i = GRID_SIZE*GRID_SIZE-1  -> TOP-RIGHT cell
 * val is used BOTH as the number drawn in that cell AND as the
 * value driving its grayscale intensity. Every new MAP: line
 * simply overwrites the grid in place (real time, no cycling
 * needed since every value carries its own explicit index).
 *
 * For backward compatibility, a bare single-integer line (no
 * "MAP:" prefix) is still accepted and cycle-filled the old way
 * (slot = totalReceivedCount % (GRID_SIZE*GRID_SIZE)).
 *
 * Any line that matches neither form (stray text, blank lines,
 * debug prints, etc.) is silently ignored -- this is how other
 * Serial output (e.g. from an LD06 LiDAR sketch) gets skipped.
 *
 * Coloring: since values aren't guaranteed to be a neat 0..n-1
 * range, color is auto-scaled every frame using the current min
 * (-> black) and max (-> white) values actually held in the grid.
 *
 * GRID_SIZE here MUST match MAP_SIZE in the Arduino sketch
 * sending the data (both currently 16, i.e. 256 values/cycle).
 * ---------------------------------------------------------
 */
